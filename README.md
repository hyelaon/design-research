...
브라우저 자동화로 후보를 모아 오면, 사람이 마음에 드는 것만 고르고(필수 개입 지점), 나머지 분류·무드보드 제작은 자동으로 처리합니다.
...
4. 프롬프트에 입력:
   ...
   ```
5. 후보가 모이면 마음에 드는 이미지를 `references/inbox/`에 저장 → **"분류"** 입력
6. 무드보드 아티팩트 확인

## 플로우
탐색(discover) → 수집(collect) → 분류(classify) → 무드보드(board) → 제작 참고(handoff)

- **2단계 큐레이션은 반드시 사람이 개입**합니다 (자동으로 넘어가지 않음)
